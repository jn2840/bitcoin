...
*Setup instructions for a gitian build of Beardcoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Beardcoin
...
- [Building Beardcoin](#building-beardcoin)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-beardcoin-host/README.md).
...
Clone the git repositories for beardcoin and gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/beardcoin/beardcoin
```
...
in the beardcoin repository under 'Fetch and build inputs' to install sources which require
...
To build Beardcoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the beardcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/beardcoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/beardcoin/beardcoin
...
For example:
```bash
URL=https://github.com/laanwj/beardcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit beardcoin=${COMMIT} --url beardcoin=${URL} ../beardcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit beardcoin=${COMMIT} --url beardcoin=${URL} ../beardcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit beardcoin=${COMMIT} --url beardcoin=${URL} ../beardcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the beardcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../beardcoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/beardcoin.git
SIGPATH=/some/root/path/beardcoin-detached-sigs.git

./bin/gbuild --url beardcoin=${BTCPATH},signature=${SIGPATH} ../beardcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/beardcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/beardcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/beardcoin-osx-build.assert
```
...
[beardcoin/gitian.sigs](https://github.com/beardcoin/gitian.sigs/) repository, or if that's not possible create a pull
