Beardcoin Core version 0.11.0 is now available from:

  <https://beardcoin.org/bin/beardcoin-core-0.11.0/>
...
backwards-compatible with pre-0.10 versions of Beardcoin Core or other software:
...
defaults to 0.00001. This will cause transactions with fewer BDC/kB fee to be
...
For example, add the following to `beardcoin.conf`:
...
related to the blockchain in the beardcoin system: the raw blocks as received over
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Beardcoin Core to delete the raw block and undo data once
...
block index and UTXO databases. The minimum was chosen so that Beardcoin Core will
...
For an example script see [beardcoin-submittx](https://github.com/laanwj/beardcoin-submittx).
...
- #6269 `95aca44` gitian: Use the new beardcoin-detached-sigs git repo for OSX signatures
...
- #5753 `0f44672` Add beardcoin logo to about screen
...
- #5809 `46bfbe7` Add beardcoin-cli man page
...
- #6116 `30dc3c1` [OSX] rename Beardcoin-Qt.app to Beardcoin-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for beardcoin-tx)
